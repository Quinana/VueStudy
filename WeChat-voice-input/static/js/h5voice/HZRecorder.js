(function (window) {
	...
	var emptyCheckCount = 0, emptydatacount = 0;
	var HZRecorder = function (stream, config) {
			...
			encodeWAV: function () {
				...
				if (sampleBits === 8) {
					...
					for (var i = 0; i < bytes.length; i++ , offset += 2) {
						var s = Math.max(-1, Math.min(1, bytes[i]));
						data.setInt16(offset, s < 0 ? s * 0x8000 : s * 0x7FFF,
							true);
					}
				}
		...
		// 播放
		this.play1 = function (fd1) {
			// audio.src = window.URL.createObjectURL(this.getBlob());
			// const blob = new Blob(fd1)
			// const blobUrl = URL.createObjectURL(fd1)
			const audio = new Audio(fd1)
			audio.play()
		};
		...
		this.save = function () {
			var fd = this.getBlob()
			const blobUrl = URL.createObjectURL(fd)
			console.log(fd)
			// var link = document.createElement('a');
			// link.style.display = 'none';
			// link.href = blobUrl;
			// link.setAttribute('download', recordedAudioFilename + '.wav');
	...
	HZRecorder.get = function (callback, config) {
		if (callback) {
			if (navigator.getUserMedia) {
						...
						function (error) {
							...
							switch (error.code || error.name) {
								...
								case 'MandatoryUnsatisfiedError':
									...
									break;
								default:
									HZRecorder.throwError('无法打开麦克风。异常信息:'
				...
				HZRecorder
					.throwErr('当前<a href="http://www.it165.net/edu/ewl/" target="_blank" class="keylink">浏览器</a>不支持录音功能。');
				return;
			}
		}
	};
